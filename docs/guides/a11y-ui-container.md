# A11y UI Container — iOS semantic grouping

`a11yUIContainer` is an iOS-only prop available on `A11y.View` and `A11y.Index`. It sets `UIAccessibilityContainerType` on the native view so VoiceOver announces the right semantic grouping when entering a section.
...
`A11y.View` is the right component when you only need container semantics and focus events — no ordering required.
...
If an ordered element also needs container semantics, use `a11yUIContainer` directly on `A11y.Index`:
...
- Setting `accessibilityLabel` on the `A11y.View` wrapper provides the container name VoiceOver reads on entry.
- `importantForAccessibility="no"` can be used on the wrapper to prevent VoiceOver from focusing the container itself, forcing it to navigate directly to children.
